<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="角色权限 (创建项目、编辑任务等)" prop="permission">
        <el-input
          v-model="queryParams.permission"
          placeholder="请输入角色权限 (创建项目、编辑任务等)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="permissionsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="角色唯一标识" align="center" prop="roleId" />
      <el-table-column label="角色名称" align="center" prop="roleName" />
      <el-table-column label="角色权限 (创建项目、编辑任务等)" align="center" prop="permission" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="角色权限 (创建项目、编辑任务等)" prop="permission">
          <el-input v-model="form.permission" placeholder="请输入角色权限 (创建项目、编辑任务等)" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getPermissions(roleId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除角色权限编号为"' + roleIds + '"的数据项？').then(function() {
        return delPermissions(roleIds);
      }).then(() => {
        this.getList();
