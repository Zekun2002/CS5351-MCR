  ...
  <div class="app-container">
          ...
          <el-col>
            <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" >
              <el-form-item label="User Name" prop="userName">
                <el-input v-model="queryParams.userName" placeholder="Please enter the user name" clearable style="width: 240px" @keyup.enter.native="handleQuery" />
              </el-form-item>
              <el-form-item label="Phone Number" prop="phonenumber">
                <el-input v-model="queryParams.phonenumber" placeholder="Please enter the phone number" clearable style="width: 240px" @keyup.enter.native="handleQuery" />
              </el-form-item>
              <el-form-item label="Status" prop="status">
                <el-select v-model="queryParams.status" placeholder="Please enter the user status" clearable style="width: 240px">
                  <el-option v-for="dict in dict.type.sys_normal_disable" :key="dict.value" :label="dict.label" :value="dict.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="Created Time">
                <el-date-picker v-model="dateRange" style="width: 240px" value-format="yyyy-MM-dd" type="daterange" range-separator="-" start-placeholder="Started Time" end-placeholder="Ended Time"></el-date-picker>
              </el-form-item>
              <el-form-item>
            ...
            <el-row :gutter="10" class="mb8">
              ...
              <el-col :span="1.5">
                <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['system:user:edit']">update</el-button>
              </el-col>
              <el-col :span="1.5">
            ...
            <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
              ...
              <el-table-column label="Created Time" align="center" prop="createTime" v-if="columns[6].visible" width="160">
                <template slot-scope="scope">
              ...
              <el-table-column label="Operation" align="center" width="160" class-name="small-padding fixed-width">
                <template slot-scope="scope" v-if="scope.row.userId !== 1">
                  <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:user:edit']">update</el-button>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="User NickName" prop="nickName">
              <el-input v-model="form.nickName" placeholder="Please enter the User NickName" maxlength="30" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Phone Number" prop="phonenumber">
              <el-input v-model="form.phonenumber" placeholder="Please enter the phonen number" maxlength="11" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col >
            <el-form-item label="Email" prop="email">
              <el-input v-model="form.email" placeholder="Please enter the e-mail" maxlength="50" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" label="password" prop="password">
              <el-input v-model="form.password" placeholder="Please enter the password" type="password" maxlength="20" show-password />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="Position">
              <el-select v-model="form.postIds" multiple placeholder="Please select the position">
          ...
          <el-col :span="12">
            <el-form-item label="Role">
              <el-select v-model="form.roleIds" multiple placeholder="Please enter the role">
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
      <el-upload ref="upload" :limit="1" accept=".xlsx, .xls" :headers="upload.headers" :action="upload.url + '?updateSupport=' + upload.updateSupport" :disabled="upload.isUploading" :on-progress="handleFileUploadProgress" :on-success="handleFileSuccess" :auto-upload="false" drag>
        ...
        <div class="el-upload__text">Drag files or click<em></em> to upload</div>
        <div class="el-upload__tip text-center" slot="tip">
          <div class="el-upload__tip" slot="tip">
            <el-checkbox v-model="upload.updateSupport" />Whether to update existing user data
          </div>
          <span>Only XLS or XLSX files are allowed for import</span>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        //   { min: 5, max: 20, message: 'Password length must be between 2 and 20 ', trigger: 'blur' },
        //   { pattern: /^[^<>"'|\\]+$/, message: "Cannot contain invalid characters:< > \" ' \\\ |", trigger: "blur" }
        // ]
      }
    };
  },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      let text = row.status === "0" ? "Activate" : "Disable";
      this.$modal.confirm('Confirm' + "\t" + text + "\t" + row.userName + '?').then(function() {
    ...
    /** 重置密码按钮操作 */
    handleResetPwd(row) {
      this.$prompt('Please enter the ' + row.userName + "'s new password'", "Tips", {
        confirmButtonText: "Submit",
        cancelButtonText: "Cancel",
        closeOnClickModal: false,
        inputPattern: /^.{5,20}$/,
        inputErrorMessage: "Password length must be between 2 and 20 ",
        inputValidator: (value) => {
          if (/<|>|"|'|\||\\/.test(value)) {
            return "Cannot contain invalid characters:< > \" ' \\\ |"
          }
        },
      }).then(({ value }) => {
          resetUserPwd(row.userId, value).then(response => {
            this.$modal.msgSuccess("Successfully updated, the new password: " + value);
          });
        }).catch(() => {});
    },
    /** 分配角色操作 */
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('Are you sure you want to delete the user data item with ID ' + userIds + "?").then(function() {
